...
require "#{File.expand_path(File.dirname(__FILE__))}/rmatic_constants.rb"
...
# Get all the status info for a given uuid (ingest). Return a list of
# hashes suitable for building a web page.
...
# Get droid info for file files in a given uuid. Return the info in a
# list of hashes suitable for building a web page.
...
def get_droid(uuid)
  ...
  (select * from identity where element='fileUtilityOutput' and mime_type not like '' group by file_id) as c
  where
  a.id=b.file_id
  and a.id=c.file_id
  order by name")
...
# Traverse path directory tree and return a list of the files. We end
# up crawling directory trees, and we have a few specific needs, so we
# needed a little method.

def traverse(path, p_flag)
  all_items = [];
  Find.find(path) { |file|
...
def run_detox(working_path, log_full_name, brief_log_full_name)
  ...
  traverse(working_path, false).each { |file|
    if (! File.file?(file))
      next
    end
...
def create_dublin_core(dc_file, dir_uuid)
  # See /opt/archivematica/xmlScripts/createDublinCore.py
  # See /home/twl8n/archivematica-read-only/includes/archivematica/xmlScripts/createDublinCore.py
